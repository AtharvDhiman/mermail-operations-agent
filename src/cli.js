...
async function main() {
  ...
  console.log('\nMermail Relayer Sentinel — CLI Management');
  console.log('-------------------------------------------------------------\n');
  ...
  switch (command) {
    case 'status': {
      ...
      for (const r of sentinel.config.relayers) {
        const flag = r.enabled ? 'ACTIVE' : 'DISABLED';
        console.log(`- [${flag}] [${r.chain.toUpperCase()}] ${r.name} (${r.id})`);
      ...
      console.log(`- Remaining: $${sentinel.budgetTracker.getRemainingBudget().toFixed(2)} USD\n`);
      break;
    }

    case 'scan': {
      console.log('[INFO] Scanning Mermail inbox for relayer alerts...');
    ...
    case 'process': {
      ...
      console.log('[INFO] Scanning and triaging alerts...');
      const incidents = await sentinel.scanForIncidents();
      ...
      for (const inc of incidents) {
        if (inc.status === SentinelStatus.AWAITING_OPERATOR_APPROVAL) {
          console.log('\n' + inc.preview);
          if (autoApprove) {
            console.log('\n[INFO] Operator approval confirmed. Dispatching PayBox replenishment...');
            ...
            console.log('- Audit Log:  Saved to Mermail drafts\n');
          } else {
            console.log('\n[WAITING] Approval required. Pass --auto-approve to execute transfer.\n');
          }
        }
      }
      break;
    ...
    default:
      console.log('Usage:');
      console.log('  node src/cli.js status                 Show configured relayers and treasury budget');
...
main().catch((err) => {
  console.error('[ERROR] Fatal Sentinel error:', err);
  process.exit(1);
});
